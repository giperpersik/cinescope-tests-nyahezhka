...
class CinescopMoviePage(BasePage):
    def __init__(self, page: Page):
        super().__init__(page)
        self.review_input = page.get_by_role("textbox", name="Написать отзыв")
        self.rating_select = page.get_by_role("combobox")
        self.submit_button = page.get_by_role("button", name="Отправить")

    @allure.step("Переход на страницу фильма: {movie_id}")
    def open_movie_page(self, movie_id: str = "65865"):
        self.open_url(f"{self.home_url}movies/{movie_id}")

    @allure.step("Оставление отзыва с текстом '{text}' и оценкой '{rating}'")
    def add_review(self, text: str, rating: str = "5"):
        self.review_input.fill(text)
        self.rating_select.click()
        self.page.get_by_role("option", name=rating).click()
        self.submit_button.click()

    @allure.step("Проверка наличия отзыва с текстом '{text}'")
